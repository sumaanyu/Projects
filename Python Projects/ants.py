...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        search_location = self.place
        if self.min_range == None and self.max_range == None:
            if search_location.bees != []:
                return random_or_none(search_location.bees)
            else:
                while search_location.entrance is not hive:
                    search_location = search_location.entrance
                    if search_location.bees != []:
                        return random_or_none(search_location.bees)
            return None
            ...
            if search_location.bees != []:
                ...
                range_counter = 1
                while range_counter <= self.max_range:
                    search_location = search_location.entrance
                    if search_location.bees != []:
                        return random_or_none(search_location.bees)
                    range_counter += 1
            ...
            range_counter = 0
            while range_counter < self.min_range:
                search_location = search_location.entrance
                range_counter += 1
            if search_location.bees != []:
                return random_or_none(search_location.bees)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            self.throw_at(self.nearest_bee(colony.hive))
            double_damage_location = self.place.exit
            while double_damage_location != None:
                if double_damage_location.ant == None:
                    double_damage_location = double_damage_location.exit
                elif double_damage_location.ant != None and double_damage_location.ant.place not in QueenAnt.doubled_ants:
                    if isinstance(double_damage_location.ant, BodyguardAnt) == True and double_damage_location.ant.ant != None:
                        double_damage_location.ant.ant.damage = 2 * double_damage_location.ant.ant.damage
                    double_damage_location.ant.damage = 2 * double_damage_location.ant.damage
                    QueenAnt.doubled_ants.append(double_damage_location.ant.place)
                    double_damage_location = double_damage_location.exit
                else:
                    double_damage_location = double_damage_location.exit

if self.impostor_queen:
            self.reduce_armor(self.armor)
        else:
            ThrowerAnt.action(self, colony)
            finder = self.place.exit
            while finder.exit:
                if finder.ant and finder.ant.container:
                    if finder.ant.ant and not finder.ant.ant.doubled:
                        finder.ant.ant.damage = finder.ant.ant.damage * 2
                        finder.ant.ant.doubled = True
                    if not finder.ant.doubled:
                        finder.ant.damage = finder.ant.damage * 2
                        finder.ant.doubled = True
                elif finder.ant and not finder.ant.doubled:
                    finder.ant.damage = finder.ant.damage * 2
                    finder.ant.doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if self.is_imposter == True and self.armor <= 0:
            self.place.remove_insect(self)
        elif self.is_imposter == False and self.armor <= 0:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    beeaction = bee.action
    def effects(colony):
        nonlocal duration
        if duration > 0:
            duration = duration - 1 
            effect(beeaction)(colony)
