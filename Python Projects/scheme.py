...
def scheme_eval(expr, env, _= None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        my_function= scheme_eval(first, env)    
        check_procedure(my_function)
        return Procedure.eval_call(my_function, rest, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    #target2 = expressions.second
    if expressions == nil:
        return None
        #what if the remainging expression is nill?
    elif expressions.first == nil:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        length_formals = len(formals)
        if len(vals) !=length_formals: 
            raise SchemeError
        for i in range(length_formals): #or while
            child.define(formals.first,vals.first)   #bind it maybe?
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        def curried_scheme_eval(y):
            return scheme_eval(y,env)
        w= operands.map(curried_scheme_eval)
        return scheme_apply(self, w, env)
        # END PROBLEM 5
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        #results = complete_eval(scheme_apply(self, operands, env))
        #return scheme_evals(results, env)
        evaluatingobject= scheme_apply(self,operands,env)
        return scheme_eval((complete_eval(evaluatingobject)),env)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6 #do something like add_primitives and make use of define used before 
        new_expression = expressions.second.first
        env.define(target, scheme_eval(new_expression, env) ) #do we need to say the first part of the second part in expressions? or just the second
        #env.define(target, scheme_eval(expressions.second, env)) 
        return target #returning the name 
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        z= LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, z)
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    #firsts = expressions.first
    #evaluated = scheme_eval(firsts,env)
    if expressions == nil: 
        return True 
...
def do_or_form(expressions, env):
    ...
    #if len(expressions)==0:
        #return False
   # y= scheme_eval(expressions.first, env)
   # if scheme_truep(y):
       # return y
   # elif len(expressions.second)==0:
       # return False
...
def make_let_frame(bindings, env):
    ...
    values= nil
    while bindings:
        check_form(bindings.first, 2,2) #first check the form
        formals= Pair(bindings.first.first, formals) #creates a Pair with all the symbols
        new_value= scheme_eval(bindings.first.second.first, env)
        values= Pair(new_value, values) #creates a Pair with all the values after evaluating them
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    targeted = expressions.first
    if scheme_symbolp(targeted):
        check_form(expressions,2,2)
        env.define(targeted, scheme_eval(expressions.second.first, env))
        return targeted
    elif isinstance(targeted, Pair) and scheme_symbolp(targeted.first):
        proc = MacroProcedure(targeted.second, expressions.second, env)
        env.define(targeted.first, proc)
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN PROBLEM 16   a call on such a procedure is executed, it is dynamically scoped
    def make_call_frame(self, args, env): #same as 12? 
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    secondformals = expressions.second 
    return MuProcedure(formals, secondformals) 
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            newfunc = lambda arg: scheme_eval(arg,env)
            new_procedure= scheme_optimized_eval(first,env)
            check_procedure(new_procedure)
            new_operands= rest.map(newfunc)
            result = scheme_apply(new_procedure,new_operands,env) 
            # END PROBLEM 20
    return result
