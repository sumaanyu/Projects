...
SquareRoot2 <- sqrt(2)  # you can press Alt -
...
# On the right hand side, in the Environment tab, click on the the dataset name (or the table symbol) on the
...
# Handle 2 functions with the same name in 2 different packages
?filter  # filter function in stats library

library(dplyr)  # load library
?filter  # filter function in dplyr library

# The order of loading the packages makes a difference, 
# i.e. the package that gets loaded last will mask the functions in packages loaded earlier.
...
write.csv(WHO.AsiaEurope, "WHO_AsiaEurope.csv", row.names=FALSE)  # row.names=TRUE will save the index of the data
